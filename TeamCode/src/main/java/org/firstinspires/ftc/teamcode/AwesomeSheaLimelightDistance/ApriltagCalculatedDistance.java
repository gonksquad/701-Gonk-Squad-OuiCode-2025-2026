...
public class ApriltagCalculatedDistance extends LinearOpMode {
            ...
            if (result != null && result.isValid()) {
                if (result.getTx() >5){
                    while (result.getTx() >5){
                        servoPosition = betweenOneAndZero(limelightServo.getPosition() + 0.05); //change limTwo from 0 to measured limit with protractor
                        limelightServo.setPosition(servoPosition);
                    ...
                    while (result.getTx() <-5){
                        servoPosition = betweenOneAndZero(limelightServo.getPosition() - 0.01);//change limTwo from 0 to measured limit with protractor
                        limelightServo.setPosition(servoPosition);
    ...
    public double getTurretAngle (double calcDist, double camPosition){
        ...
        turretAngle = betweenLimAndLim(turretAngle,1,0);
        return turretAngle;
    }
}